import {RPC} from '/@cryptix/grpc-web';
...
import {isMobile} from '/@cryptix/ux/cryptix-ux.js';
export * from '/@cryptix/ux/cryptix-ux.js';

window.__testI18n = (test)=>i18n.setTesting(!!test);

class CryptixWalletHeader extends BaseElement{
	static get styles(){
		return css`
			:host{display:block}
			.container{
...
CryptixWalletHeader.define("cryptix-wallet-header")

class CryptixWalletApp extends FlowApp {
	...
	render(){
		...
		return html`
		${isMobile?'':html`<!--cryptix-wallet-header></cryptix-wallet-header-->`}
		<cryptix-wallet .walletMeta='${meta}' reloadonlock="true" hidefaucet="true"></cryptix-wallet>
		`
	}
	...
	firstUpdated(){
		...
		this.wallet = this.renderRoot.querySelector("cryptix-wallet");
...
CryptixWalletApp.define("cryptix-wallet-app");
